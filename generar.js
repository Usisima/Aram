...
for (const [id, l] of Object.entries(CATALOGO)) {
  {
    ...
    /* Quién lo usa y cómo. Es lo que hace visible que un libro no es de una
       materia: el mismo Spivak es la bibliografía básica de dos cálculos. */
    const donde = materias
      .filter((m) => (m.libros || []).some((x) => x.id === id))
      .map((m) => {
        const suyo = m.libros.find((x) => x.id === id);
        const como = suyo.papel === "complementario" ? "complementaria" : "básica";
        return `<a href="${materia(m.id)}">${m.nombre}</a> (${como})`;
      });
    ...
    if (donde.length)
      suyo.push(`        <p class="tambien">Bibliografía de ${donde.join(", ")}</p>`, "");
